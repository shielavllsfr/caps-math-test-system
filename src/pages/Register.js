...
export default function Register() {
    ...
    const handleOtherInfo = (e) => {
        ...
        if (inputID === 'email') {
            ...
            setEmailState({ error: true, message: 'Email must end with @bulsu.edu.ph' });
            ...
            if (e.target.value.length < 6) setPassStat({ error: true, message: 'password must be at least 6 characters' })
    ...
    const checkPass = (e) => {
        if (userInfo.password === e.target.value || (userInfo.password === '' && e.target.value === '')) setPassStat({ error: false, message: '' })
        else setPassStat({ error: true, message: 'Password does not match'})
    ...
    return(
        ...
        <Grid className={classes.root}>
            ...
            <Grid>
                <img src={ImageImporter.register_img1} className={classes.imgPos} alt="register_img1"/>
            </Grid>
            ...
            <Grid className={classes.borderPos}>
                <Grid className={classes.border}>
                    ...
                    <Grid className={classes.leftSide} sx={{ display: 'flex', marginTop: '2.5vh' }}>
                        <Typography
                        variant="h5">
                            Student ID
                        </Typography>
                    ...
                    <FormGroup className={classes.leftSide}>
                        <FormControlLabel control={<Checkbox onClick={handleTerms}/>} label="I Agree with terms and conditions" />
                    </FormGroup>
                    <Button 
                    ...
                    sx={{ marginTop: '2.5vh' }}>
                    { registerState.registering ? <CircularProgress color='secondary' size={20} sx={{ marginRight: '2vh' }}/> : null}
                    REGISTER
                    </Button>
