...
export default function AdminDashboard(){
                ...
                .then((testResults) => {
                    ...
                    .then((pollData) => {
                        ...
                        pollData.docs.forEach((pollVotesData) =>{
                            if (pollVotesData.id === 'abuse') abuseVotes = pollVotesData.data().votes
                            else if(pollVotesData.id === 'drugs') drugVotes = pollVotesData.data().votes
                        })
                        setPollRes({
                            abuse: Math.round((abuseVotes / (drugVotes + abuseVotes)) * 100),
                            drugs: Math.round((drugVotes / (drugVotes + abuseVotes)) * 100)
                        })
                    })
                })
            })
    ...
    const applyControlChanges = () => {
        ...
        if (dataControl.cba || dataControl.cite || dataControl.cit || dataControl.coed) {
            
            if (dataControl.cba) {
                if (dataControl.firstYear || dataControl.secondYear || dataControl.thirdYear || dataControl.forthYear) {
                    if (dataControl.firstYear) {
                    ...
                    newPassedValue += cbaTotal.fyPassed + cbaTotal.syPassed + cbaTotal.tyPassed + cbaTotal.foyPassed;
                    newFailedValue += cbaTotal.fyFailed + cbaTotal.syFailed + cbaTotal.tyFailed + cbaTotal.foyFailed;
                }
            }
            ...
            if (dataControl.cite) {
                if (dataControl.firstYear || dataControl.secondYear || dataControl.thirdYear || dataControl.forthYear) {
                    ...
                    newPassedValue += citeTotal.fyPassed + citeTotal.syPassed + citeTotal.tyPassed + citeTotal.foyPassed;
                    newFailedValue += citeTotal.fyFailed + citeTotal.syFailed + citeTotal.tyFailed + citeTotal.foyFailed;
                }
            }
            ...
            if (dataControl.cit) {
                if (dataControl.firstYear || dataControl.secondYear || dataControl.thirdYear || dataControl.forthYear) {
                    ...
                    newPassedValue += citTotal.fyPassed + citTotal.syPassed + citTotal.tyPassed + citTotal.foyPassed;
                    newFailedValue += citTotal.fyFailed + citTotal.syFailed + citTotal.tyFailed + citTotal.foyFailed;
                }
            }
            ...
            if (dataControl.coed) {
                if (dataControl.firstYear || dataControl.secondYear || dataControl.thirdYear || dataControl.forthYear) {
                    ...
                    newPassedValue += coedTotal.fyPassed + coedTotal.syPassed + coedTotal.tyPassed + coedTotal.foyPassed;
                    newFailedValue += coedTotal.fyFailed + coedTotal.syFailed + coedTotal.tyFailed + coedTotal.foyFailed;
                }
            }
        }else {
            if (dataControl.firstYear) {
                currentTotal += cbaTotal.firstY + citeTotal.firstY + citTotal.firstY + coedTotal.firstY;
                newPassedValue += cbaTotal.fyPassed + citeTotal.fyPassed + citTotal.fyPassed + coedTotal.fyPassed;
                newFailedValue += cbaTotal.fyFailed + citeTotal.fyFailed + citTotal.fyFailed + coedTotal.fyFailed;
            }
            if (dataControl.secondYear){
                currentTotal += cbaTotal.secondY + citeTotal.secondY + citTotal.secondY + coedTotal.secondY;
                newPassedValue += cbaTotal.syPassed + citeTotal.syPassed + citTotal.syPassed + coedTotal.syPassed;
                newFailedValue += cbaTotal.syFailed + citeTotal.syFailed + citTotal.syFailed + coedTotal.syFailed;
            }
            if (dataControl.thirdYear) {
                currentTotal += cbaTotal.thirdY + citeTotal.thirdY + citTotal.thirdY + coedTotal.thirdY;
                newPassedValue += cbaTotal.tyPassed + citeTotal.tyPassed + citTotal.tyPassed + coedTotal.tyPassed;
                newFailedValue += cbaTotal.tyFailed + citeTotal.tyFailed + citTotal.tyFailed + coedTotal.tyFailed;
            }
            if (dataControl.forthYear) {
                currentTotal += cbaTotal.forthY + citeTotal.forthY + citTotal.forthY + coedTotal.forthY;
                newPassedValue += cbaTotal.foyPassed + citeTotal.foyPassed + citTotal.foyPassed + coedTotal.foyPassed;
                newFailedValue += cbaTotal.foyFailed + citeTotal.foyFailed + citTotal.foyFailed + coedTotal.foyFailed;
            }
        }
                ...
                <Grid>
                    <Grid className={classes.flex} style={{ marginLeft: '4vh' }}>
                        <Grid className={classes.border} style={{ textAlign: 'center',  }}>
                            <Typography variant='h5' style={{ marginTop: '3vh' }}>Total Students</Typography>
                            <img alt='std_icon' src={ImageImporter.std_icon}  style={{ margin: '2vh 0vh' }}/>
                        ...
                        <Grid className={classes.border} style={{textAlign: 'center'}}>
                            <Grid style={{ borderBottom: '1px solid gray'}}>
                                ...
                                <Typography variant='h3' className={classes.percentage}>{pollRes.drugs}%</Typography>
                            </Grid>
                            <Grid style={{ borderTop: '1px solid gray'}}>
                                <Typography className={classes.title}>MATH IS</Typography>
                                <Typography>Mental Abuse To Humans</Typography>
                                <Typography variant='h3' className={classes.percentage}>{pollRes.abuse}%</Typography>
                            </Grid>
                        </Grid>
                        <Grid className={classes.border} style={{ overflow: 'auto' }}>
                            <Grid style={{ display: 'flex', marginTop: '1vh' }}>
                                <Button size='small' color='success' style={{ height: '2.5vh' }} onClick={applyControlChanges}>Apply</Button>
                                <Typography variant='subtitle2' style={{ marginLeft: '5.5vh' }}>Data Control</Typography>
                            </Grid>
                            <FormGroup style={{ marginLeft: '2vh'}}>
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.selectAll} id='SELECT ALL' onClick={dataControlChanged}/>} label="Select All" />
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.cba} id='CBA' onClick={dataControlChanged}/>} label="CBA" />
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.cite} id='CITE' onClick={dataControlChanged}/>} label="CITE" />
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.cit} id='CIT' onClick={dataControlChanged}/>} label="CIT" />
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.coed} id='COED' onClick={dataControlChanged}/>} label="CoED" />
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.firstYear} id='1ST' onClick={dataControlChanged}/>} label="1st year" />
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.secondYear} id='2ND' onClick={dataControlChanged}/>} label="2nd year" />
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.thirdYear} id='3RD' onClick={dataControlChanged}/>} label="3rd year" />
                              <FormControlLabel control={<Checkbox size='small' checked={dataControl.forthYear} id='4TH' onClick={dataControlChanged}/>} label="4th year" />
                            </FormGroup>
                        </Grid>
                    </Grid>
                    <Grid className={classes.flex} style={{ marginTop: '-2vh', justifyContent: 'center' }}>
                        <Grid style={{ marginRight: '3vh' }}>
                            <Chart
                              ...
                              data={[
                                ...
                                ['1st Year', resByYear.firstYearPassed, resByYear.firstYearFailed],
                                ['2nd Year', resByYear.secondYearPassed, resByYear.secondYearFailed],
                                ['3rd Year', resByYear.thirdYearPassed, resByYear.thirdYearFailed],
                                ['4th Year', resByYear.forthYearPassed, resByYear.forthYearFailed],
                              ]}
                              options={{
                                  chart:{
