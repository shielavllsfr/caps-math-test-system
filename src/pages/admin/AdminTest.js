...
export default function AdminTest() {
  ...
  const handleClose = (event, reason) => {
    ...
    const deleteTestQuestion = (e) => {
      ...
      asignTestValues();
      updateTestState(question, questionField, questionFieldLength, IDS, correctAnswer, a, b, c, d, aLength, bLength, cLength, dLength, prevCorrectAnswer);
    }
    ...
    const correctAnsChanged = (e) => {
      ...
      const questionRoot = e.target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
      if (prevCorrectAnswer[IDS.indexOf(questionRoot.id)] !== null) {
        ...
        prevCorrectAnswer[IDS.indexOf(questionRoot.id)].setAttribute('class', 'MuiRadio-root MuiRadio-colorPrimary MuiButtonBase-root MuiRadio-root MuiRadio-colorPrimary PrivateSwitchBase-root css-vqmohf-MuiButtonBase-root-MuiRadio-root')
        prevCorrectAnswer[IDS.indexOf(questionRoot.id)].children[1].lastChild.setAttribute('class', 'MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-1hhw7if-MuiSvgIcon-root')
      }
      ...
      const newQuestion = (
          <Grid key={randomID} id={randomID} style={{ marginBottom: '2vh' }}>
            <Grid style={{ display: 'flex'}}>
              <TextField variant='outlined' label="Statement" id='statement' style={{ width: '110vh' }} onChange={valueChanged}></TextField>
              <IconButton style={{ marginTop: '-0.5vh', marginLeft: '1vh' }} onClick={deleteQuestion}>
                <img src={ImageImporter.delete_icon} alt='delete icon' id='surveyDelete' style={{ height: '5vh', width: '4vh' }}/>
              </IconButton>
            </Grid>
          </Grid>
        );
    ...
    const generateTestObj = (randomID) => {
      const newTestQuestion = (
        <Grid key={randomID} id={randomID} style={{ marginBottom: '5vh' }}>
          <Grid style={{ display: 'flex'}}>
            <TextField variant='outlined' label='Question' id='question' style={{ width: '110vh' }} onChange={valueChanged}></TextField>
            <IconButton style={{ marginTop: '-0.5vh', marginLeft: '1vh'  }} onClick={deleteTestQuestion}>
              <img src={ImageImporter.delete_icon} alt='delete icon' style={{ height: '5vh', width: '4vh' }}/>
            </IconButton>
          </Grid>
          <Typography style={{ marginTop: '2vh' }}>CHOICES:</Typography>
          <Grid style={{ display: 'flex', width: '90%' }}>
            <Grid>
              <TextField variant='outlined' label='A:' id='a' className={classes.choicesWidth} onChange={valueChanged}></TextField>
              <TextField variant='outlined' label='B:' id='b' className={classes.choicesWidth} onChange={valueChanged}></TextField>
            </Grid>
            <Grid>
              <TextField variant='outlined' label='C:' id='c' className={classes.choicesWidth} onChange={valueChanged}></TextField>
              <TextField variant='outlined' label='D:' id='d' className={classes.choicesWidth} onChange={valueChanged}></TextField>
            </Grid>
            <Grid>
    ...
    const generateRecommendationObj = (id, from) => {
      const obj = (
        <Grid key={id} id={id} style={{ marginBottom: '2vh' }}>
          <Grid style={{ display: 'flex'}}>
            <TextField variant='outlined' label="Recommendation" id={from} style={{ width: '50vh' }} onChange={valueChanged}></TextField>
            <IconButton style={{ marginTop: '-0.5vh', marginLeft: '1vh' }} onClick={deleteQuestion}>
              <img src={ImageImporter.delete_icon} alt='delete icon' id={from + 'Delete'} style={{ height: '5vh', width: '4vh' }}/>
            </IconButton>
          </Grid>
        </Grid>
      )
    ...
    const updateTestState = (questionObj, question, questionLength, id, correctAns, a, b, c, d, aLength, bLength, cLength, dLength, prevCorrectAnswer) => {
      setTest({
    ...
    const asignSurveyValues = (from, panel) => {
      if (from === 'survey') {
        for (let i = 0; i < IDS.length; i++) {
          let inputRoot = questionContainer.current.children[i].firstChild.firstChild.children[1].firstChild;
          inputRoot.value = fieldValue[IDS.indexOf(questionContainer.current.children[i].id)]
        }
      }else if(from === 'recommendation'){
        if (panel === 'sufficient') {
          for (let i = 0; i < IDS.length; i++) {
            let inputRoot = sufficientContainer.current.children[i].firstChild.firstChild.children[1].firstChild;
            inputRoot.value = sufiValue[IDS.indexOf(sufficientContainer.current.children[i].id)]
          }
        }else if(panel === 'insufficient')
          for (let i = 0; i < insuID.length; i++) {
            let inputRoot = insufficientContainer.current.children[i].firstChild.firstChild.children[1].firstChild;
            inputRoot.value = InsuValue[insuID.indexOf(insufficientContainer.current.children[i].id)]
          }
      }
    ...
    const asignTestValues = () => {
      ...
      for (let i = 0; i < IDS.length; i++) {
        let inputRoot = testQuestionContainer.current.children[i].firstChild.firstChild.children[1].firstChild;
        ...
        for (let x = 0; x < choicesRoot.children.length; x++) {
          ...
          for (let z = 0; z < choiceDiv.children.length; z++) {
            ...
            if (choiceRuntimes < 5) {
              if (choiceRuntimes === 1) choiceContainer.firstChild.value = a[index]
              else if (choiceRuntimes === 2) choiceContainer.firstChild.value = b[index]
              else if (choiceRuntimes === 3) choiceContainer.firstChild.value = c[index]
              else if (choiceRuntimes === 4) choiceContainer.firstChild.value = d[index]
            }else {
              if (correctAnswer[index] === 'a') {
                ...
                choiceContainer.children[0].firstChild.setAttribute('class', 'MuiRadio-root MuiRadio-colorPrimary MuiButtonBase-root MuiRadio-root MuiRadio-colorPrimary PrivateSwitchBase-root Mui-checked css-vqmohf-MuiButtonBase-root-MuiRadio-root')
                choiceContainer.children[0].firstChild.children[1].lastChild.setAttribute('class', 'MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-11zohuh-MuiSvgIcon-root')
                ...
                choiceContainer.children[2].firstChild.setAttribute('class', 'MuiRadio-root MuiRadio-colorPrimary MuiButtonBase-root MuiRadio-root MuiRadio-colorPrimary PrivateSwitchBase-root Mui-checked css-vqmohf-MuiButtonBase-root-MuiRadio-root')
                choiceContainer.children[2].firstChild.children[1].lastChild.setAttribute('class', 'MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-11zohuh-MuiSvgIcon-root')
                console.log(choiceContainer.children[2].firstChild.children[1].lastChild)
                ...
                choiceContainer.children[1].firstChild.setAttribute('class', 'MuiRadio-root MuiRadio-colorPrimary MuiButtonBase-root MuiRadio-root MuiRadio-colorPrimary PrivateSwitchBase-root Mui-checked css-vqmohf-MuiButtonBase-root-MuiRadio-root')
                choiceContainer.children[1].firstChild.children[1].lastChild.setAttribute('class', 'MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-11zohuh-MuiSvgIcon-root')
                ...
                choiceContainer.children[3].firstChild.setAttribute('class', 'MuiRadio-root MuiRadio-colorPrimary MuiButtonBase-root MuiRadio-root MuiRadio-colorPrimary PrivateSwitchBase-root Mui-checked css-vqmohf-MuiButtonBase-root-MuiRadio-root')
                choiceContainer.children[3].firstChild.children[1].lastChild.setAttribute('class', 'MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-11zohuh-MuiSvgIcon-root')
              }
              choiceRuntimes = 0;
            }
          }
    ...
    const submitData = (e) => {
      ...
      if (e.target.id === 'submit-survey') {
        ...
        if (send) {
          ...
          .catch((e) => {
            setSnackMessage({ type: 'error', message: 'Something went wrong, check out the console for more info' })
            handleSnack();
            console.log(e)
            setLoading(false)
          })
        }else {
          setSnackMessage({ type: 'error', message: 'Please don\'t leave any field empty' })
          handleSnack();
          setLoading(false)
        }
        ...
        IDS = test.id;
        for (let i = 0; i < IDS.length; i++) {
          if (test.questionFieldLength[i] === 0 || test.aLength[i] === 0 || test.bLength[i] === 0 || test.cLength[i] === 0 || test.dLength[i] === 0 || test.correctAnswer[i] === '') {
            send = false;
          }
          console.log('loop')
        }
        ...
        if (send) {
          ...
          .catch((e) => {
            setSnackMessage({ type: 'error', message: 'Something went wrong, check the console for more info' })
            handleSnack();
            console.log(e);
            setLoading(false)
          })
        ...
        if (send) {
          ...
          .then(() => {
            ...
            .then(() => {
              ...
              setSnackMessage({ type: 'error', message: 'Something went wrong trying to save the \'insufficient\' recommendations, check the console for more details' })
              handleSnack()
          ...
          .catch((e) => {
            setSnackMessage({ type: 'error', message: 'Something went wrong trying to save the \'sufficient\' recommendations, check the console for more details' });
            handleSnack();
            console.error(e.message)
            setLoading(false)
          })
    ...
    const initSurvey = () => {
      ...
      const initRecommendation = () => {
        if ((value !== 3)) {
          ...
          .then((sufficientData) => {
            ...
            .then((insufficientData) => {
              if (sufficientData.data() !== undefined) {
                sufficientData.data().sufficientValues.forEach((sufficientValue) => {
                  let suffiID = Math.random();
                  let newRecoObj = generateRecommendationObj(suffiID, 'sufficient');
              ...
              if (insufficientData.data() !== undefined) {
                insufficientData.data().insufficientValues.forEach((insufficientValue) => {
                let insuId = Math.random();
                let newRecoObj = generateRecommendationObj(insuId, 'insufficient');
            ...
            .catch((e) => {
              setSnackMessage({ type: 'error', message: 'Something went wrong fetching \'insufficient\' values' })
              handleSnack()
              console.log(e)
            })
          ...
          .catch((e) => {
            setSnackMessage({ type: 'error', message: 'Something went wrong fetching \'sufficient\' values' })
            handleSnack()
          })
        }
      }
    ...
    <Grid>
        ...
        <Box  
          sx={{ flexGrow: 1, bgcolor: 'background.paper', display: 'flex', height: '100%', marginTop: '15vh' }}
        >
          <Tabs
          ...
          > 
            <Tab disabled={disableTab} className={classes.tabLabel} label="SURVEY RESULT" onClick={initSurveyRes} {...a11yProps(0)}/>
            <Tab disabled={disableTab} className={classes.tabLabel} label="UPDATE SURVEY" onClick={initSurvey} {...a11yProps(1)} />
            <Tab disabled={disableTab} className={classes.tabLabel} label="UPDATE TEST" onClick={initTest} {...a11yProps(2)} />
            <Tab disabled={disableTab} className={classes.tabLabel} label="UPDATE RECOMMENDATION" onClick={initRecommendation} {...a11yProps(3)} />
          </Tabs>
          <TabPanel value={value} index={0}>
            <Grid style={{ width: '160vh', height: '78vh', overflow: 'auto', border: '1px solid black' }}>
              <Grid style={{ display: 'flex', justifyContent: 'center', marginTop: '2vh' }}>
                <Typography variant='h5'><b>SURVEY RESULT</b></Typography>
              </Grid>
          ...
          <TabPanel value={value} index={1}>
            {initLoad ? (
              <Grid style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '78vh',  width: '150vh'  }}>
              ...
              <Grid style={{ 
                ...
                marginLeft: '8vh',
                overflow: 'auto' }}>
                <Typography variant='h5' style={{ marginTop: '3vh' }}><b>SURVEY</b></Typography>
                <Button variant='contained' color='success' style={{ marginTop: '2vh' }} onClick={addQuestion} id="survey-statement">ADD SURVEY STATEMENT</Button>

                <Container style={{ marginTop: '5vh', textAlign: 'left', marginLeft: '20vh'}} ref={questionContainer}>
          ...
          {initLoad ? (
            ...
            <Grid style={{ 
              ...
              marginLeft: '8vh',
              overflow: 'auto' }}>
              <Typography variant='h5' style={{ marginTop: '3vh' }}><b>TEST QUESTIONS</b></Typography>
              <Button variant='contained' color='success' style={{ marginTop: '2vh' }} onClick={addQuestion} id="test-question">ADD NEW QUESTION</Button>
              
              <Container style={{ marginTop: '5vh', textAlign: 'left', marginLeft: '20vh'}} ref={testQuestionContainer}>
          ...
          <Grid style={{ 
              ...
              marginLeft: '8vh',
              overflow: 'auto' }}>
              <Typography variant='h5' style={{ marginTop: '3vh' }}><b>RECOMMENDATION</b></Typography>

              
                <Grid style={{ marginTop: '5vh', display: 'flex', justifyContent: 'center'}}>
                  <Grid className={classes.recommendationPanel} style={{ marginRight: '10vh' }}>
                    <Typography variant='h6'>Sufficient Knowledge Reccomendation/s:</Typography>
                    <Button variant='contained' id='sufficient' onClick={addQuestion} color='success' style={{ marginTop: '1vh', marginBottom: '4vh' }}>New Recommendation</Button>

                    <Container ref={sufficientContainer}>

                    </Container>

                  </Grid>
                  <Grid className={classes.recommendationPanel}>
                    <Typography variant='h6'>Insufficient Knowledge Reccomendation/s:</Typography>
                    <Button variant='contained' id='insufficient' onClick={addQuestion} color='success' style={{ marginTop: '1vh', marginBottom: '4vh' }}>New Recommendation</Button>

                    <Container ref={insufficientContainer}>
