...
export default function Login() {
            ...
            if (localStorage.getItem('uid') !== '') {
                ...
                .then((user) => {
                    if (user.data().rememberCredential) {
                        firebase
                        .auth()
                        .signInWithEmailAndPassword(localStorage.getItem('email'), localStorage.getItem('pass'))
                        .then(() => {
    ...
    const inputChanged = (e) => {
        if (e.target.id === 'email') {
            ...
            setEmailState({ error: true, message: 'Email must ends with @bulsu.edu.ph' })
            ...
            if (e.target.value.length < 6) {
                setPassState({ error: true, message: 'Password must be at least 6 characters'});
            }else {
    ...
    return(
        <Grid className={classes.root}>
                <Grid>
                    <img className={classes.img} src={ImageImporter.login_img1} alt="login_img1"/>
                </Grid>
                <Grid className={classes.borderPos}>
                    <Grid className={classes.border}>
                        ...
                        <Grid sx={{ display: 'flex', alignItems: 'center', marginTop: '1vh'}}>

                            <FormGroup className={classes.leftText}>
                                <FormControlLabel control={<Checkbox onClick={rememberUser}/>} label="Remeber me" />
                            </FormGroup>
                        ...
                        <Grid className={classes.center} style={{ marginTop: '10vh' }}>
                            ...
                            onClick={login}>
                                { loginInfo.signing ? <CircularProgress color='secondary' size={20} sx={{ marginRight: '2vh' }}/> : null }
                                LOGIN
                                </Button>
                        </Grid>
                    </Grid>
